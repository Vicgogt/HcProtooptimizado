...
# ============================================================
# CONFIGURACIÓN BASE HCR
# El menú hcr permite modificar estos valores después.
# ============================================================
PORT="8080"
PORT_SET="false"

MAX_DOWNLOAD_FRAME="1032"
DOWNLOAD_POLL_TIMEOUT="25s"
...
Options:
  ...
  --transport <mode>  Server transport. Default: auto
                      ...
                      auto  accepts TLS and non-TLS HCR on same port
...
Base HCR tuning:
  ...
  max-sessions             64
  max-sessions-per-ip      32
  max-download-frame     1032
  download-poll-timeout   25s
  session-stats-interval   0
EOF
}
...
acquire_install_lock() {
    exec 9<"${SYSTEMD_DIR}" ||
        fail "The systemd unit directory could not be opened for locking."

    flock -n 9 ||
        fail "Another HCR Server installer is already running."
}
...
validate_binary_identity() {
    ...
    [[ "${output}" =~ ^hcr-server[[:space:]]version[[:space:]][0-9]+\.[0-9]+\.[0-9]+(\ -\ Patch\ [1-9][0-9]*)?$ ]] ||
...
validate_tls_pair() {
    ...
    private_public_key="$(
        openssl pkey -in "${TLS_KEY_PATH}" -passin pass: -pubout 2>/dev/null
    )" ||
...
ExecStart=${BINARY_PATH} --listen :${PORT} --target 127.0.0.1:22 --transport ${TRANSPORT}${tls_arguments} --max-connections ${MAX_CONNECTIONS} --max-sessions ${MAX_SESSIONS} --max-sessions-per-ip ${MAX_SESSIONS_PER_IP} --max-download-frame ${MAX_DOWNLOAD_FRAME} --download-poll-timeout ${DOWNLOAD_POLL_TIMEOUT} --session-stats-interval ${SESSION_STATS_INTERVAL}
Restart=on-failure
RestartSec=5s
TimeoutStopSec=15s
KillSignal=SIGTERM
...
verify_service_health() {
    ...
    initial_pid="$(
        systemctl show --property=MainPID --value "${SERVICE_NAME}.service"
    )"

    [[ "${initial_pid}" =~ ^[1-9][0-9]*$ ]] ||
        fail "The service did not report a running process."
...
install_service() {
    ...
    [ -L "${UNIT_LINK_PATH}" ] ||
        ln -s -- "${UNIT_SOURCE_PATH}" "${UNIT_LINK_PATH}"
    ...
    echo "  Max sessions/IP:        ${MAX_SESSIONS_PER_IP}"
    echo "  Max download frame:     ${MAX_DOWNLOAD_FRAME}"
    echo "  Download poll timeout:  ${DOWNLOAD_POLL_TIMEOUT}"
    echo "  Session stats interval: ${SESSION_STATS_INTERVAL}"
    echo "  LimitNOFILE:            8192"
}
